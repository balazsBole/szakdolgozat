...
A példányok metrikáit (\ref{sec:metrikak} pont) nyomon lehet követni az erre a célra létrehozott Grafana oldalon (\ref{todo} ábra). Az oldal elérhető a  \textit{Spring metrics} menüpont alatt.

\Aref{todo} ábrán csak a Grafana oldal legfelső néhány panele látható, az instance-okra lebontott legfontosabb mérőszámokkal:
...
egy-egy érdekesebb problémásabb felület
 ...
 meg a user keresés leütöm callback meg  - ez jó lehet még


\section{Eureka}
\Aref{sec:metrikak}. pontban megemlített szerviz felderítésre az Eureka szervert használom. A helpdesk backend és az e-mail kliens induláskor beregisztálják magukat az Eureka szervizbe.
