...
Ezért a megismételt a JMeter tesztben a backendet közvetlenül az IP-címén keresztül értem el. Az új mérés --\ref{tabl:without_nginx} táblázat-- adataiból látszódik,
...
\begin{itemize}
	\item hogy az átlagos és a medián válaszidő, valamint áteresztőképesség nem változott,
...
Ugyanez látszódik \aref{fig:without_nginx_min_median_max_over_time} ábrán is, a teszt első háromnegyedében a backend néhány kérésre azonnal válaszol, míg a többségre --medián-- csak sokkal később.
...
\begin{itemize}
	\item hogy a különböző tesztesetekre adott válaszidő kkülönbözik (\ref{fig:without_nginx_percentil} ábra),
...
Ezért a megismételt a JMeter tesztben a backendben megháromszoroztam a HikariCP pooljának a méretét. A mérési eredmények --\ref{tabl:hikari_30}~táblázat-- nem mutatnak eltérést az első esethez (\ref{sec:peak_1_instance}) képest.
...
Ezért a megismételt a JMeter tesztben a backendből három példányt indítottam el. Az eredmények így összemérhetőek maradtak \aref{sec:hikari}. ponntal, hiszen összességében ugyanannyi kapcsolat van a backend és az adatbázis között.
...
\begin{itemize}
	\item hogy az átlagos válaszidő 6 másodper alá esett,
